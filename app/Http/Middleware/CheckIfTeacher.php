...
class CheckIfTeacher
{
     ...
     *
     * @return \Illuminate\Http\RedirectResponse [description]
     */
    private function checkIfUserIsTeacher($request, Closure $next)
    {
        if (Auth::user()) {
            if (Auth::user()->hasRole(\App\Models\User::ROLE_TEACHER)) {
                return $next($request);
            } else {
    ...
    {
        if (Auth::user()->hasRole(\App\Models\User::ROLE_GUEST)) {
            return $this->respondToUnauthorizedRequest($request);
        }

        if (!$this->checkIfUserIsTeacher($request, $next)) {
            return $this->respondToUnauthorizedRequest($request);
        }

    ...
    <nav
        ...
        <div class="flex flex-col justify-center items-center text-sm md:text-md text-center w-1/2">
            ...
            <template x-if="$store.learnView.type == 'exerciseBlock.start'">
                <div>
                    <div v-html="$store.learn.currentTopic.name"></div>
                    <div v-html="$store.learn.currentLearningSequence.title"></div>
                </div>
            </template>
            <template x-if="$store.learnView.type == 'question.solve'">
                <div class="w-full bg-gray-100 rounded-lg relative overflow-hidden">
                    <div class="absolute top-0 w-full">{{ percentCompleted }}</div>
                </div>
...
const {currentLearningSequence, }
const percentCompleted = computed(() => 10 + '%')
